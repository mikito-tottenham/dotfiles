...
- この環境の gws プロファイル対応は、`gws-account taskell` = `mikito.ebishima@taskell.ai`（taskell.ai 作業、Taskell Management 共有ドライブ含む）、`gws-account ges-claude` = `mikito.ebishima@yoake-entertainment.jp`（yoake 系）、`gws-account twinplanet` = `mikito.ebishima@twinplanet.co.jp`（TWIN PLANET 作業）、`gws-account amicitia` = `mikito.ebishima@amicitia.jp`（amicitia.jp 作業）とすること
- 個人 Gmail アカウント `mikito.bail@gmail.com` には専用の gws プロファイルが無いが、`gws-account amicitia` がそのカレンダーに owner 権限を持つため、カレンダー操作は amicitia プロファイル経由で行うこと（2026-09-09 実測）
