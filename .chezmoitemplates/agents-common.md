...
- **認証は例外なく 1Password 経由とすること**。ツール独自の認証ストアへ実値を保存する経路（`gh auth login` の keyring、平文の token file、1Password 管理外の SSH 秘密鍵など）は、1Password が使えないときの代替経路としても採用しないこと。ただし 1Password を正本とし 1 コマンドで再生成できる materialized file（`opmaterialize restore` の復元物、`ghrun --refresh` が生成する `~/.config/op/injected/github.env`）はこの禁止の対象外とする（dotfiles ADR-0063）。SSH が要る場合は 1Password の SSH agent を使う
...
- `git commit` は、作業単位が完了し検証（テスト・リンクチェック・機密情報の混入確認など）を終えた時点で、ユーザーへの確認を挟まずに実行すること（2026-09-22 ユーザー指示「確認しないでもう保存して今後は」。2026-09-09 の「コミットも例外にしない」を上書き。dotfiles ADR-0065）。対象はその作業で意図して変更したファイルに限り、無関係な変更や他セッションの変更を巻き込まないこと。既定ブランチ上で作業している場合は作業用のローカルブランチを切ってからコミットしてよいこと（ローカルのブランチ作成は確認不要）。コミットハッシュ・メッセージ・対象ファイルは作業結果の報告に含めること
- `git push`・PR 作成・マージ・ブランチ削除・タグ・force push など、リモートや GitHub の状態を変更する操作は、引き続きユーザーがその操作をチャットで明示的に承認・指示した場合だけ実行すること（2026-09-09 ユーザー指示。プッシュは例外にしない）。承認は操作ごと・セッションごとに必要で、過去の承認を別の操作や次回作業へ一般化しないこと。承認前は差分と PR 本文案の提示までに留め、ユーザーが承認したら該当コマンドだけを単独で実行すること
