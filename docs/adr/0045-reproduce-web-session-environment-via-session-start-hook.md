...
- `scripts/bootstrap-web` を追加し、(1) chezmoi の用意、(2) `.chezmoiignore` 整合確認後の `chezmoi apply`、(3) first-party スキル（repo `skills/`）と公開 third-party スキル（`gws-*`, `grill-me`）の再インストールを冪等に行う。
