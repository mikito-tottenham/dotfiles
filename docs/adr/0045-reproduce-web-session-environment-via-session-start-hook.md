...
- 実行結果は `$BOOTSTRAP_WEB_STATUS`（既定 `~/.cache/bootstrap-web/status.json`）に機械可読で書き、`overall_success`・`first_party_failed`・`third_party_installed`・`third_party_skipped` を残す。サマリも標準出力へ出す（静かな成功・静かな部分失敗を作らない）。
