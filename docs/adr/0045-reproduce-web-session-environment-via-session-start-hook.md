...
- runner スキルが叩く CLI のうち、認証不要（`ghq`）または env 由来の認証で headless 完走できるもの（`codex`＝`CODEX_AUTH_JSON` から `~/.codex/auth.json` を materialize、`gemini`＝実行時 `GEMINI_API_KEY`）を best-effort で導入する。`op`・`gh`／`gh skill`・`gws`・`grok` は入手経路ブロック・OAuth headless 不可・契約必須のため自動導入対象外とし、各 runner の `SKILL.md` / docs-only manifest の手動セットアップに委ねる。
- 実行結果は `$BOOTSTRAP_WEB_STATUS`（既定 `~/.cache/bootstrap-web/status.json`）に機械可読で書き、`overall_success`・`failure_stage`・`failure_reason`・`first_party_failed`・`third_party_installed`・`third_party_skipped`・`runner_cli_installed`・`runner_cli_skipped` を残す。`chezmoi apply` 失敗等の早期致命終了でも `EXIT` trap が失敗ステータスを書く。サマリも標準出力へ出す（静かな成功・静かな部分失敗を作らない）。
