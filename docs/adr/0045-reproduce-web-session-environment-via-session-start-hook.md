...
- dotfiles 以外のリポジトリ（workspace 等）のセッションでも再現したい場合は、environment の **Setup script**（repo 非依存・全セッション・キャッシュ対象）から dotfiles を clone して `bootstrap-web` を実行する。Setup script はキャッシュされ結果がスナップショットに焼き込まれるため、**`BOOTSTRAP_WEB_SKIP_OP=true` を付けて op restore を skip** し、secret をスナップショットに残さず on-demand restore（`opmaterialize restore`、token は env 済み）に委ねる。CLI/chezmoi/skill の導入だけをキャッシュさせ、secret は毎回フレッシュにする住み分けとする。Setup script は環境 UI に置く最小スニペット（clone → exec `bootstrap-web`）に留め、実ロジックは git 管理の `bootstrap-web` に集約する。
