...
- web セッションでは egress 許可リストに `api.openai.com` / `chatgpt.com` が含まれないため（2026-06-22 時点で両者とも 403）、Codex CLI はモデル API に到達できず実行不可。`bootstrap-web` は Codex CLI を best-effort 導入するが、実行可否は別問題として残る。したがって `dot_*`（Codex 管理対象）の変更を **web セッション内で Codex に委譲することはできず**、ローカル環境の Codex で行うか、egress 許可に `api.openai.com`（ChatGPT 経路を使う場合は `chatgpt.com`）を追加する必要がある。
