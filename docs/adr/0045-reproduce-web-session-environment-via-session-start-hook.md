...
- web セッションのコンテナは直接の外部 egress を持たず、通信は Claude Code 管理 proxy 経由になる（`CLAUDE_CODE_PROXY_RESOLVES_HOSTS`）。一般 HTTP では `api.openai.com` / `chatgpt.com` に到達できない（2026-06-22 実測。挙動はセッション/環境依存で、proxy 経由で 403 になる場合と、proxy 有無（`--noproxy '*'` 含む）いずれも DNS 解決不可になる場合がある。いずれも素の egress では openai に到達しない点は共通）。**ただし** Claude Code の `openai-codex` プラグインの companion runtime（`CODEX_COMPANION_SESSION_ID`）経由では Codex CLI（0.141.0、ChatGPT ログイン済み）が実行でき、現に web セッション内で Codex を呼んでレビュー・調査を実行できることを実測で確認した（`codex exec` 成功）。したがって **素のコンテナ外部 egress 経路では openai に到達不可だが、openai-codex プラグイン companion runtime 経由なら Codex CLI は実行可能**。プラグインを介さない直接の `api.openai.com` 通信に依存する処理は依然不可。`dot_*`（Codex 管理対象）の変更を web で Codex に委譲する場合は、この plugin proxy 経路が有効なセッションに限る。
- このリポジトリの管理責任は Codex が持つため、本 ADR と `AGENTS.md` の例外条項は Codex によるレビュー対象として扱う。

## 追記 (2026-06-22, Claude Code / claude-sonnet-4-6)

- 当初は「web では `api.openai.com` / `chatgpt.com` が 403 で Codex CLI 実行不可・web セッション内で Codex に委譲できない」と記していたが、web セッション内での実測で訂正した。コンテナ直接 egress は遮断（セッション/環境により proxy 経由 403 または DNS 解決不可。実測ラウンドで差が出た）だが、`openai-codex` プラグイン companion runtime 経由では `codex exec` が成功し、本セッションでも Codex によるレビューを複数回実行できた。
